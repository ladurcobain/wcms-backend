...
{
    private $table = "tp_legal";
    private $field = "legal_id";

    private $menuId  = "61";
    private $menuLog = "Zona Integritas Dasar Hukum";
    ...
    public function getAll(Request $request) {
        ...
        if($satker != "") {
            ...
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('legal_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('legal_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('legal_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('legal_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->count();
            }
        }
        else {
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->where('legal_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('legal_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('legal_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('legal_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('is_deleted', 0)->count();
            }
        }
        
        $arr = Init::initIntegrityLegal(1, $tmp);
        $rst = (($cnt > 0)?1:0);
        $data = array(
            'total' => $cnt,
            'lists' => $arr,
    ...
    public function getSingle(Request $request) {
        ...
        $arr  = Init::initIntegrityLegal(0, $data);
    ...
    public function insertData(Request $request) {
        ...
        if($validate == "") {
            ...
            $rst = DB::table($this->table)
                ->insertGetId([
                    "legal_text_in" => $text_in,
                    "legal_text_en" => $text_en,
                    "legal_satker"  => Dbase::dbGetFieldById('tm_satker', 'satker_name', 'satker_id', $satker),
                    "satker_id"     => $satker,
                    "created_at"    => $now,
                    "updated_at"    => $now,

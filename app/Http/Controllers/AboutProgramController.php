...
{
    private $table = "tp_program";
    private $field = "program_id";

    private $menuId  = "28";
    private $menuLog = "Tentang Kami Program Kerja JA";
    ...
    public function getAll(Request $request) {
        ...
        if($satker != "") {
            ...
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('program_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('program_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('program_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('program_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->count();
            }
        }
        else {
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->where('program_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('program_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('program_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('program_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('is_deleted', 0)->count();
            }
        }
        
        $arr = Init::initAboutProgram(1, $tmp);
        $rst = (($cnt > 0)?1:0);
        $data = array(
            'total' => $cnt,
            'lists' => $arr,
    ...
    public function getSingle(Request $request) {
        ...
        $arr  = Init::initAboutProgram(0, $data);
    ...
    public function insertData(Request $request) {
        ...
        if($validate == "") {
            ...
            $rst = DB::table($this->table)
                ->insertGetId([
                    "program_text_in"  => $text_in,
                    "program_text_en"  => $text_en,
                    "program_satker"   => Dbase::dbGetFieldById('tm_satker', 'satker_name', 'satker_id', $satker),
                    "satker_id"        => $satker,
                    "created_at"       => $now,
                    "updated_at"       => $now,

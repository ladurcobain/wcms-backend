...
{
    private $table = "tp_doctrin";
    private $field = "doctrin_id";

    private $menuId  = "22";
    private $menuLog = "Tentang kami Doktrin Adhyaksa";
    ...
    public function getAll(Request $request) {
        ...
        if($satker != "") {
            ...
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('doctrin_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('doctrin_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('doctrin_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('doctrin_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->count();
            }
        }
        else {
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->where('doctrin_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('doctrin_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('doctrin_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('doctrin_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('is_deleted', 0)->count();
            }
        }
        
        $arr = Init::initAboutdoctrin(1, $tmp);
        $rst = (($cnt > 0)?1:0);
        $data = array(
            'total' => $cnt,
            'lists' => $arr,
    ...
    public function getSingle(Request $request) {
        $id   = $request->doctrin_id;
        $data = DB::table($this->table)->where($this->field, $id)->get();
        $arr  = Init::initAboutdoctrin(0, $data);
    ...
    public function insertData(Request $request) {
        ...
        if($validate == "") {
            ...
            $rst = DB::table($this->table)
                ->insertGetId([
                    "doctrin_text_in"   => $text_in,
                    "doctrin_text_en"   => $text_en,
                    "doctrin_satker"    => Dbase::dbGetFieldById('tm_satker', 'satker_name', 'satker_id', $satker),
                    "satker_id"         => $satker,
                    "created_at"        => $now,
                    "updated_at"        => $now,

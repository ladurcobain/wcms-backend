...
{
    private $table = "tp_supervision";
    private $field = "supervision_id";

    private $menuId  = "67";
    private $menuLog = "Zona Integritas Penguatan Pengawasan";
    ...
    public function getAll(Request $request) {
        ...
        if($satker != "") {
            ...
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('supervision_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('supervision_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('supervision_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('supervision_id', 'DESC')->get();
                $cnt = DB::table($this->table)->whereIn('satker_id', $arrSatker)->where('is_deleted', 0)->count();
            }
        }
        else {
            if($request->keyword != "") {
                $tmp = DB::table($this->table)->where('supervision_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('supervision_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('supervision_text_in', 'like', '%'.$request->keyword.'%')->where('is_deleted', 0)->count();
            }    
            else {
                $tmp = DB::table($this->table)->where('is_deleted', 0)->take($limit)->skip($offset)->orderBy('supervision_id', 'DESC')->get();
                $cnt = DB::table($this->table)->where('is_deleted', 0)->count();
            }
        }
        
        $arr = Init::initIntegritySupervision(1, $tmp);
        $rst = (($cnt > 0)?1:0);
        $data = array(
            'total' => $cnt,
            'lists' => $arr,
    ...
    public function getSingle(Request $request) {
        $id   = $request->supervision_id;
        $data = DB::table($this->table)->where($this->field, $id)->get();
        $arr  = Init::initIntegritySupervision(0, $data);
    ...
    public function insertData(Request $request) {
        ...
        if($validate == "") {
            ...
            $rst = DB::table($this->table)
                ->insertGetId([
                    "supervision_text_in"  => $text_in,
                    "supervision_text_en"  => $text_en,
                    "supervision_satker"   => Dbase::dbGetFieldById('tm_satker', 'satker_name', 'satker_id', $satker),
                    "satker_id"            => $satker,
                    "created_at"           => $now,
                    "updated_at"           => $now,
